...
export type Code = {
  ...
  expiresAt?: string;
  /**
   * 官方页把这组码标成「N 月版本」但**没给失效日**时填 'YYYY-MM'。
   * 为什么要单独一个字段而不是直接填 expiresAt：expiresAt 的定义是「官方标注的失效日」，
   * Klook 那 30 组银行码官方只写了「每月更換，此為 9 月版本」，没写哪天失效——
   * 替它填 09-30 等于替 Klook 宣布一个它没公布的日期。
   * 这个字段只用来在月底之后把「✓ 已核對」降成「⚠ 此為 9 月版，10 月版尚未核對」，
   * 不触发自动下架（码可能还有效，只是我们不知道）。
   */
  officialVersion?: string;
  /**
   * 下面四个字段把 restriction 那句自由文本拆成可以分组的轴。
   * 为什么要拆：2026-09-21 补完码之后 Klook 有 30 组、KKday 有 30 组，平铺成一个列表，
   * 拿玉山卡的读者得逐条扫 30 行才知道哪条跟自己有关。而这正是台湾券站读者的第一个问题。
   *
   * 三条纪律：
   *   1. 值是从 restriction 抽出来的，**restriction 仍然是唯一真源** —— 它写了全部条件
   *      （限用次数、是否要先点活动链接、折抵上限），这四个字段只是它的索引。
   *   2. 抽不出来就留空，不要硬归一类。iHerb 那条「部分特殊商品不參與」不属于任何轴，
   *      它就该三个字段都空着。
   *   3. 改 restriction 的时候记得同步这里，否则分组会和条件对不上。
   */
  /** 付款方式：限定的银行／卡别／支付工具。 */
  card?: string;
  /** 限定下单的星期，0=周一 … 6=周日。KKday 有 9 组这种码，是「今天能用哪几组」的依据。 */
  weekday?: number;
  /** 限定的商品类别（日本鐵路商品、飯店商品…）。 */
  category?: string;
  /** 限定的客户身分。 */
  ...
  },
  {
    ...
    codes: [
      ...
      },
      {
        ...
        category: 'Zone 4 Walkabout Pass',
        verifiedAt: '2026-09-21',
        source: 'https://www.kkday.com/zh-tw/blog/25229/kkday-bank-coupon',
      ...
      },
      {
        ...
        category: 'Premier Walkabout Pass',
        verifiedAt: '2026-09-21',
        source: 'https://www.kkday.com/zh-tw/blog/25229/kkday-bank-coupon',
        expiresAt: '2026-10-11',
        commissionable: 'unknown',
